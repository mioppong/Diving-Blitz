...
function water(event)
    ...
    if (event.phase == "began") then
        ...
        composer.gotoScene( "level3" , {effect = "fade", time = 500})
    end
end
...
function onCollision( event )
    ...
    if (event.phase =="began") then
        ...
        composer.gotoScene( "level4", {effect = "fade", time= 500});
...
function scene:show( event )
    ...
    tmr = timer.performWithDelay( 2000, function()  -- Works even for rotated
            ...
            maxRadius = math.floor( (maxRadius * 10^2) + 0.5) / (10^2)
               if( (circle.x - maxRadius) > right ) then physics.stop()--[[triesCounter()--]] composer.gotoScene( "level3", {effect = "fade", time= 1000}); return false end
               if( (circle.x + maxRadius) < left) then physics.stop()--[[triesCounter()--]] composer.gotoScene( "level3", {effect = "fade", time= 1000}); return false end
               if( (circle.y + maxRadius) < top ) then physics.stop()--[[triesCounter()--]] composer.gotoScene( "level3", {effect = "fade", time= 1000}); return false end
               if( (circle.y - maxRadius) > bottom ) then physics.stop()--[[triesCounter()--]] composer.gotoScene( "level3", {effect = "fade", time= 1000}); return false end
