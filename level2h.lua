...
function water(event)
    ...
    if (event.phase == "began") then
        ...
        composer.gotoScene( "level2h" , {effect = "fade", time = 500})
    end
end
...
local sceneGroup = self.view
    ...
    background = display.newImage( "pictures/Levels/two.png", halfW,halfH );
...
function scene:show( event )
    ...
    if ( phase == "will" ) then
       ...
       physics.start();
        ...
        transition.to(point, {time = 10000, iterations = 0, x=25 , transition = easing.continuousLoop})
        transition.to(floaty, {time = 10000, iterations = 0, x=25 , transition = easing.continuousLoop})
    ...
    tmr = timer.performWithDelay( 2000, function()  -- Works even for rotated
            ...
            maxRadius = math.floor( (maxRadius * 10^2) + 0.5) / (10^2)
               if( (circle.x - maxRadius) > right ) then physics.stop()--[[triesCounter()--]] composer.gotoScene( "level2h", {effect = "fade", time= 1000}); return false end
               if( (circle.x + maxRadius) < left) then physics.stop()--[[triesCounter()--]] composer.gotoScene( "level2h", {effect = "fade", time= 1000}); return false end
               if( (circle.y + maxRadius) < top ) then physics.stop()--[[triesCounter()--]] composer.gotoScene( "level2h", {effect = "fade", time= 1000}); return false end
               if( (circle.y - maxRadius) > bottom ) then physics.stop()--[[triesCounter()--]] composer.gotoScene( "level2h", {effect = "fade", time= 1000}); return false end
