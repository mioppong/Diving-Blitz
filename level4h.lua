...
function onCollision( event )
    ...
    if (event.phase =="began") then
        ...
        composer.gotoScene( "soon", {effect = "fade", time= 500});
...
local sceneGroup = self.view
    ...
    background = display.newImage( "pictures/Levels/four.png", halfW,halfH );
...
function scene:show( event )
    ...
    if ( phase == "will" ) then
       ...
       circle = display.newImageRect(  "pictures/penguin.png",  50, 50 )
        ...
        transition.to(point, {time = 1000, iterations = 0, x=25 , transition = easing.continuousLoop})
        transition.to(floaty, {time = 1000, iterations = 0, x=25 , transition = easing.continuousLoop})
    ...
    tmr = timer.performWithDelay( 2000, function()  -- Works even for rotated
            ...
            maxRadius = math.floor( (maxRadius * 10^2) + 0.5) / (10^2)
               if( (circle.x - maxRadius) > right ) then physics.stop()--[[triesCounter()--]] composer.gotoScene( "level4h", {effect = "fade", time= 1000}); return false end
               if( (circle.x + maxRadius) < left) then physics.stop()--[[triesCounter()--]] composer.gotoScene( "level4h", {effect = "fade", time= 1000}); return false end
               if( (circle.y + maxRadius) < top ) then physics.stop()--[[triesCounter()--]] composer.gotoScene( "level4h", {effect = "fade", time= 1000}); return false end
               if( (circle.y - maxRadius) > bottom ) then physics.stop()--[[triesCounter()--]] composer.gotoScene( "level4h", {effect = "fade", time= 1000}); return false end
