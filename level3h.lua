...
function water(event)
    ...
    if (event.phase == "began") then
        ...
        composer.gotoScene( "level3h" , {effect = "fade", time = 500})
    end
end
...
function onCollision( event )
    ...
    if (event.phase =="began") then
        ...
        composer.gotoScene( "level4h", {effect = "fade", time= 500});
...
local sceneGroup = self.view
    ...
    background = display.newImage( "pictures/Levels/three.png", halfW,halfH );
...
function scene:show( event )
    ...
    if ( phase == "will" ) then
       ...
       circle = display.newImageRect(  "pictures/penguin.png",  50, 50 )
        ...
        physics.addBody(point, "static", {bounce =2})
         ...
         transition.to(point, {time = 9000, iterations = 0, x=25 , transition = easing.continuousLoop})
        transition.to(floaty, {time = 9000, iterations = 0, x=25 , transition = easing.continuousLoop})
    ...
    tmr = timer.performWithDelay( 2000, function()  -- Works even for rotated
            ...
            maxRadius = math.floor( (maxRadius * 10^2) + 0.5) / (10^2)
               if( (circle.x - maxRadius) > right ) then physics.stop()--[[triesCounter()--]] composer.gotoScene( "level3h", {effect = "fade", time= 1000}); return false end
               if( (circle.x + maxRadius) < left) then physics.stop()--[[triesCounter()--]] composer.gotoScene( "level3h", {effect = "fade", time= 1000}); return false end
               if( (circle.y + maxRadius) < top ) then physics.stop()--[[triesCounter()--]] composer.gotoScene( "level3h", {effect = "fade", time= 1000}); return false end
               if( (circle.y - maxRadius) > bottom ) then physics.stop()--[[triesCounter()--]] composer.gotoScene( "level3h", {effect = "fade", time= 1000}); return false end
